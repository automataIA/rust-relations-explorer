...
pub fn run_cli(cli: Cli) -> i32 {
    match cli.command {
        Commands::Build {
            ...
            let mut theme = match dot_theme.as_str() {
                "dark" => DotTheme::Dark,
                _ => DotTheme::Light,
            };
            let mut rankdir = match dot_rankdir.as_str() {
                "TB" => RankDir::TB,
                _ => RankDir::LR,
            };
            let mut splines = match dot_splines.as_str() {
        ...
        Commands::Query { query } => match query {
            QueryCommands::ConnectedFiles {
                ...
                if fmt == "json" {
                    let out: Vec<String> =
                        results.into_iter().map(|p| p.display().to_string()).collect();
            ...
            QueryCommands::FunctionUsage {
                ...
                let graph = if let Some(p) = graph_path {
                    ...
                    let res =
                        match KnowledgeGraph::build_from_directory(std::path::Path::new(&path)) {
                            Ok(g) => g,
                            Err(e) => {
                                eprintln!("Build failed: {e}");
                                if no_ignore {
                ...
                let dir = match direction.as_str() {
                    "callees" => crate::query::UsageDirection::Callees,
                    _ => crate::query::UsageDirection::Callers,
                };
            ...
            QueryCommands::ModuleCentrality {
                ...
                if fmt == "json" {
                    ...
                    let body: Vec<Vec<String>> = rows
                        ...
                        .collect();
                    let table =
                        crate::utils::table::render(&["Module", "In", "Out", "Total"], &body);
                    println!("{table}");
                }
                0
            }
            QueryCommands::TraitImpls {
                ...
                if fmt == "json" {
                    ...
                    let out: Vec<Row> = rows
                        .into_iter()
                        .map(|(p, t)| Row { path: p.display().to_string(), r#type: t })
                        .collect();
                    match serde_json::to_string_pretty(&out) {
                        Ok(s) => println!("{s}"),
                        Err(e) => {
                    ...
                    let body: Vec<Vec<String>> =
                        rows.into_iter().map(|(p, t)| vec![p.display().to_string(), t]).collect();
                ...
                if fmt == "json" {
                    ...
                    let body: Vec<Vec<String>> = rows
                        ...
                        .map(|(p, id, name, kind, vis)| {
                            vec![p.display().to_string(), id, name, kind, vis]
                        })
                        .collect();
                    let table = crate::utils::table::render(
